...
async function parsePDFBuffer(buffer, barnId, dateHint) {
  ...
  for (const line of lines) {
    ...
    const glPattern = /(\d+)\s+([A-Z]+(?:\/[A-Z]+)?)\s+(\d{3,4})\s+\$?([\d,]+\.?\d*)/g;
    let gm;
    let foundGL = false;
    while ((gm = glPattern.exec(line)) !== null) {
      ...
      const price  = parseFloat(gm[4].replace(/,/g, ""));
      if (!head || isNaN(weight) || isNaN(price) || weight < 50 || price < 5) continue;
      lots.push({ head, color, class: currentClass, weight, price,
        sale_type: currentSaleType, sale_date: currentDate || "unknown",
        per_head: isPerHead(currentSaleType, currentClass), barn_id: barnId });
      foundGL = true;
    ...
    // ── Rock Creek representative sales ──
    // "Rush City Black Steer Calves 348 2 517.50"
    // ends with: WEIGHT HEAD PRICE
