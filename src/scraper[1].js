...
// Override sale type based on class — e.g. lambs/goats are always Sheep & Goats
// regardless of what section header they appeared under
function overrideSaleType(saleType, cls) {
  const c = cls.toUpperCase();
  if (c.match(/LAMB|EWE|WOOL|HAIR|GOAT|KID/)) return "Sheep & Goats";
  if (c.match(/BRED/) && c.match(/EWE|SHEEP/)) return "Sheep & Goats";
  return saleType;
}
  ...
  $("h3, h4, table").each((_, el) => {
    ...
    $(el).find("tr").each((_, row) => {
      ...
      lots.push({ head, color, class: cls, weight, price,
        sale_type: overrideSaleType(currentSaleType, cls), sale_date: currentDate || "unknown",
        per_head: isPerHead(overrideSaleType(currentSaleType, cls), cls), barn_id: barnId });
    });
  });
...
function parsePipestonePage(html, defaultSaleType, barnId) {
  ...
  // Insert newlines before each lot so we can parse them one at a time.
  const fullText = $("body").text();
  const normalized = fullText.replace(/(\d+\s+head)/gi, "\n$1");
  const lines = normalized.split("\n").map(l => l.trim()).filter(Boolean);

  let currentSaleType = defaultSaleType;
  let currentClass    = defaultSaleType === "Slaughter"     ? "COW"       :
                        defaultSaleType === "Sheep & Goats" ? "LAMB"      :
                        defaultSaleType === "Fed Cattle"    ? "FAT STEER" : "STEER";
  ...
  for (const line of lines) {
    ...
    // Class label lines like "Steers:", "Heifers:", "Lambs:"
    if (upper.match(/^STEERS?\s*:/))  { currentClass = "STEER";     continue; }
    if (upper.match(/^HEIFERS?\s*:/)) { currentClass = "HEIFER";    continue; }
    if (upper.match(/^COWS?\s*:/))    { currentClass = "COW";       continue; }
    if (upper.match(/^BULLS?\s*:/))   { currentClass = "BULL";      continue; }
    if (upper.match(/^LAMBS?\s*:/))   { currentClass = "LAMB";      continue; }
    if (upper.match(/^EWES?\s*:/))    { currentClass = "EWE";       continue; }
    if (upper.match(/^GOATS?\s*:|^KIDS?\s*:/)) { currentClass = "GOAT"; continue; }
    if (upper.includes("HOLSTEIN STEER")) currentClass = "HOL STEER";
    if (upper.includes("HOL HEIFER"))     currentClass = "HOL HEIFER";

    // Match lot: "N head....WEIGHT#.....$?PRICE [per head] [COLOR]"
    const m = line.match(/(\d+)\s+head[\s.]+([\d]+)#[\s.]+\$?([\d,]+\.?\d*)\s*(per\s+head)?/i);
    if (!m) continue;

    const head    = parseInt(m[1], 10);
    const weight  = parseFloat(m[2]);
    const price   = parseFloat(m[3].replace(/,/g, ""));
    const perHead = !!m[4] || isPerHead(currentSaleType, currentClass);
    const rest    = line.slice(m.index + m[0].length).trim();
    const colorM  = rest.match(/^(BLK|RED|BWF|RWF|MIXED|CHAR|HOL|XBRED|MIX|JERS)\b/i);
    const color   = colorM ? colorM[1].toUpperCase() : "UNK";

    if (head && weight > 0 && price > 0) {
      lots.push({ head, color, class: currentClass, weight, price,
        sale_type: overrideSaleType(currentSaleType, currentClass), sale_date: saleDate || "unknown",
...
async function parsePDFBuffer(buffer, barnId, dateHint) {
  ...
  if (barnId === "rockcreek") {
    ...
    while ((rm = rcPattern.exec(text)) !== null) {
      ...
      lots.push({ head, color, class: cls, weight, price,
        sale_type: overrideSaleType(saleType, cls), sale_date: currentDate || "unknown",
        per_head: isPerHead(overrideSaleType(saleType, cls), cls), barn_id: barnId });
    }
    ...
    while ((bm = brPattern.exec(text)) !== null) {
      ...
      lots.push({ head, color, class: cls, weight, price,
        sale_type: overrideSaleType("Bred Cows", cls), sale_date: currentDate || "unknown",
  ...
  for (const line of lines) {
    ...
    while ((gm = glPattern.exec(line)) !== null) {
      ...
      lots.push({ head, color, class: currentClass, weight, price,
        sale_type: overrideSaleType(currentSaleType, currentClass), sale_date: currentDate || "unknown",
        per_head: isPerHead(overrideSaleType(currentSaleType, currentClass), currentClass), barn_id: barnId });
      foundGL = true;
    }
    if (foundGL) continue;
...
async function saveLots(lots, barnId, sourceUrl) {
  ...
  const { error } = await supabase.from("livestock_lots")
    .upsert(rows, { onConflict: "barn_id,sale_date,sale_type,class,color,head,weight,price", ignoreDuplicates: true });
